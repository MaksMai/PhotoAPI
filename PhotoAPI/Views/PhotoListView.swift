...
extension PhotoListView: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) { // большое фото
        let photo = vm.getPhoto(at: indexPath.row)
        delegate?.selectedPhoto(photo: photo)
    }
    
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) { // безконечная прокрутка
